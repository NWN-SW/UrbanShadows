...
void main() {
    ...
    if(nEvent == X2_ITEM_EVENT_ONHITCAST){
        ...
        //Combo Stuff
        int nCombo = GetLocalInt(oPC, "WERECAT_COMBO");

        if(nCombo >= 8)
        {
            // if so, prepare damage to be dished out
           nCombo = nCombo * 5;
           effect eDamage = EffectDamage(d6(nCombo), DAMAGE_TYPE_SLASHING, DAMAGE_POWER_NORMAL);
           effect eVis = EffectVisualEffect(VFX_COM_BLOOD_CRT_RED);
           effect eLink = EffectLinkEffects(eVis, eDamage);
            // apply it
           ApplyEffectToObject(DURATION_TYPE_INSTANT, eLink, oEnemy);
           FloatingTextStringOnCreature("BONUS DAMAGE!", oPC, TRUE);
           SetLocalInt(oPC, "WERECAT_COMBO", 0);
        }
        else
        {
            nCombo = nCombo + 1;
            SetLocalInt(oPC, "WERECAT_COMBO", nCombo);
        }
    }
