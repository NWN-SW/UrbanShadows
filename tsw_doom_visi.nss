...
void main()
{
    ...
    int nCheck = GetLocalInt(OBJECT_SELF, "DOOMSEER_VISION");
    if(nCheck == 1)
    {
    ...
    if(GetHasFeat(DOOM_PROPHECY_FIRE, OBJECT_SELF) && GetSpellId() == 952)
    {
    ...
    else if(GetHasFeat(DOOM_PROPHECY_COLD, OBJECT_SELF) && GetSpellId() == 953)
    {
    ...
    else if(GetHasFeat(DOOM_PROPHECY_ELEC, OBJECT_SELF) && GetSpellId() == 954)
    {
    ...
    SetLocalInt(OBJECT_SELF, "DOOMSEER_VISION", 1);
    DelayCommand(fDuration, DeleteLocalInt(OBJECT_SELF, "DOOMSEER_VISION"));

    //Apply VFX
    ApplyEffectToObject(DURATION_TYPE_INSTANT, eBoom, OBJECT_SELF);
    DelayCommand(0.25, ApplyEffectToObject(DURATION_TYPE_INSTANT, eBoom, OBJECT_SELF));
    DelayCommand(0.5, ApplyEffectToObject(DURATION_TYPE_INSTANT, eBoom, OBJECT_SELF));
    ...
    //Start Custom Spell-Function Block
        //Get damage
        string sTargets = "AOE";
        int nDamage = GetSeventhLevelDamage(oTarget, 5, sTargets);
