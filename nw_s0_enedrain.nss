...
void main()
{
    ...
    int nDuration = (GetCasterLevel(OBJECT_SELF) + GetLevelByClass(CLASS_TYPE_BLACKGUARD)) / 3;
    ...
    effect eArrow = EffectVisualEffect(246);
    ...
    if (GetIsReactionTypeFriendly(oTarget) == FALSE)
    {
        ...
        if(MyResistSpell(OBJECT_SELF, oTarget) == FALSE)
        {
            ...
            //----------------------------------------------------------------------
            int nDamage = GetNinthLevelDamage(oTarget, nCasterLvl, nMetaMagic, "Single");
...
void RunImpact(object oTarget, object oCaster, int nMetaMagic)
{
    ...
    //--------------------------------------------------------------------------
    if (GZGetDelayedSpellEffectsExpired(SPELL_ENERGY_DRAIN, oTarget, oCaster))
    {
        return;
    }
