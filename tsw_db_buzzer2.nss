...
void Nui_Tablet_Window2(object oPlayer) {
    ...
    json jInputPannel = JsonArray(); {
        ...
        json jgInputGroup = JsonArray(); {
            json jrInputBody = JsonArray(); {
                ...
                //Spacer:
                //jrInputBody = JsonArrayInsert(jrInputBody, NuiHeight(NuiSpacer(), 1.0f));
                ...
                json jgUserNameGroup = JsonArray(); {
                    json jrInputBody = JsonArray();  {
                        ...
                        json jrImageRow = JsonArray();  {
                            //Spacer
                            //jrImageRow = JsonArrayInsert(jrImageRow, NuiHeight(NuiSpacer(), 2.0f));

                            //Message Edit Box
                            json jCDEditBox = JsonArray(); {
                                jCDEditBox = CreateTextEditBox (jCDEditBox , "desc_Placeholder", "buzzer_unsrame_input",
                                    1000, TRUE, 280.0, 23.0, "buzzer_usrname_tooltip");
                    ...
                    jgUserNameGroup    = NuiHeight(jgUserNameGroup, 36.0f);
                        ...
                        json jrImageRow = JsonArray();  {
                            ...
                            json jCDEditBox = JsonArray(); {
                                jCDEditBox = CreateTextEditBox (jCDEditBox , "desc_Placeholder", "buzzer_msg_input",
                                                  180, TRUE, 280.0, 140.0, "buzzer_msg_tooltip");
                                jCDEditBox = NuiRow(jCDEditBox);
                            } 
							jrImageRow = JsonArrayInsert(jrImageRow, jCDEditBox);
                            jrImageRow = NuiRow(jrImageRow);
				...
				json jbSendBuzz = NuiImage(
					...
					jbSendBuzz = NuiId(jbSendBuzz, "nui_cmark_b01");
				...
				json jrMessageSendButtonRow = JsonArray(); {	;
					...
					jrMessageSendButtonRow = JsonArrayInsert (jrMessageSendButtonRow, NuiSpacer ());
            ...
            jgInputGroup    = NuiGroup(NuiCol(jrInputBody), TRUE, NUI_SCROLLBARS_NONE);
            jgInputGroup    = NuiHeight(jgInputGroup, 352.0f);
		...
		json jgSendGroup = JsonArray(); {
			json jrInputBody = JsonArray(); {
				...
				json jgUserIconGroup = JsonArray(); {
					...
					} jrIconRow = JsonArrayInsert(jrIconRow, jbSendBuzz);
				}
				//Set the group. We are giving it a fixed size so that the layout wont be affected by message lengths.
				jgUserIconGroup    = NuiGroup(NuiCol(jrIconRow), FALSE, NUI_SCROLLBARS_NONE);
				jgUserIconGroup    = NuiHeight(jgUserIconGroup, 110.0f);
			...
			jrInputBody = JsonArrayInsert(jrInputBody, jgUserIconGroup);
			jrInputBody = NuiCol(jrInputBody);
			
		} jCol = JsonArrayInsert(jCol, jgSendGroup);
    ...
    json jMsgPannel = JsonArray();
    {
        ...
        json jgMsgButtonRow = JsonArray(); {
            json jrMainRow = JsonArray(); {
                ...
                json jcImageColumn = JsonArray();
                {
                    json jARWL = NuiImage(
                        ...
                        jCMrk1 = NuiId(jCMrk1, "nui_buzzer_cmark_b01");
            ...
            //Set the group. We are giving it a fixed size so that the layout wont be affected by message lengths.
            jgMsgButtonRow    = NuiGroup(NuiRow(jrMainRow), TRUE, NUI_SCROLLBARS_NONE);
            jgMsgButtonRow    = NuiHeight(jgMsgButtonRow, 40.0f);
    ...
    string nBzzIconID;
    string sRefBuzzerIcon = "BzzrIcn25"; //This needs to be changed to an sql character database draw if a character posted before
    ...
    string sBuzzUsrName = "Buzzer Username!";
    ...
    NuiSetBind (oPlayer, nToken, "nui_buzzer_cmark_b01", JsonBool (TRUE));
