...
// End of Spell Cast Hook
    ...
    effect eExplode = EffectVisualEffect(VFX_FNF_DISPEL_DISJUNCTION);
    effect eVis = EffectVisualEffect(VFX_FNF_GAS_EXPLOSION_MIND);
    effect eDur = EffectVisualEffect(1114);
    effect eDam;
    string sTargets;
    ...
    while (GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
        {
            ...
            if(nDamage > 0)
            {
                ...
                float fTime = GetExtendSpell(12.0);
                effect eHeal = EffectHeal(nFinalDamage);
                effect eMind = EffectVisualEffect(VFX_IMP_HEAD_MIND);
                DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eDur, oTarget, fTime));
                DelayCommand(fTime, ApplyEffectToObject(DURATION_TYPE_INSTANT, eHeal, oTarget));
                DelayCommand(fTime, ApplyEffectToObject(DURATION_TYPE_INSTANT, eMind, oTarget));
            }
        }
