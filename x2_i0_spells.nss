...
void CheckAndApplyThunderingRage(int nRounds)
{
    ...
    {
        string sOld = GetEventScript(OBJECT_SELF, EVENT_SCRIPT_CREATURE_ON_HEARTBEAT);
        SetEventScript(OBJECT_SELF, EVENT_SCRIPT_CREATURE_ON_HEARTBEAT, "pc_thunderrage");
        float fDur = RoundsToSeconds(nRounds);
        DelayCommand(fDur, ExecuteScript("pc_resethtbt"));
    }
}
