...
void main() {
    ...
    if(nEvent == X2_ITEM_EVENT_ONHITCAST){
        ...
        if(sItemTagOnHitTwoHander != "OH_1H") {
            return;
        }
        ...
        int nRolledDamage = d10(2) + nSTR;

        //Buff damage if they have Use Poison feat
        if(GetHasFeat(FEAT_USE_POISON, OBJECT_SELF))
        {
            nRolledDamage = nRolledDamage * 2;
        }
        ...
        while(oTarget != OBJECT_INVALID) {
            ...
            if(GetIsEnemy(oTarget, oPC) && oTarget != oEnemy)
            {
               ...
               effect eVis = EffectVisualEffect(283);
