/*
Custom Contagion chaining spell by Alexander G.*/
...
// End of Spell Cast Hook
    ...
    int nCasterLevel = GetCasterLevel(OBJECT_SELF) + GetLevelByClass(CLASS_TYPE_BLACKGUARD);
    int nDamage;
    int nDamStrike;
    int nNumAffected = 0;
    int nMetaMagic = GetMetaMagicFeat();
    //Declare lightning effect connected the casters hands
    effect eLightning = EffectBeam(VFX_BEAM_DISINTEGRATE, OBJECT_SELF, BODY_NODE_HAND);;
    ...
    nDamage = GetFourthLevelDamage(oFirstTarget, nCasterLevel, nMetaMagic, "AOE");
    ...
    nDamage = GetFortDamage(oFirstTarget, nDC, nDamage);
    ...
    if (spellsIsTarget(oFirstTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF))
    {
        ...
        if (!MyResistSpell(OBJECT_SELF, oFirstTarget))
        {
            ...
            eDamage = EffectDamage(nDamage, DAMAGE_TYPE_ACID);
    ...
    eLightning = EffectBeam(VFX_BEAM_DISINTEGRATE, oFirstTarget, BODY_NODE_CHEST);
    ...
    while (GetIsObjectValid(oTarget) && nCnt < nCasterLevel)
    {
        ...
        if (oTarget != oFirstTarget && spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF) && oTarget != OBJECT_SELF)
        {
            ...
            {
            eLightning = EffectBeam(VFX_BEAM_DISINTEGRATE, oHolder, BODY_NODE_CHEST);
            }
            else
            {
                // * April 2003 trying to make sure beams originate correctly
                effect eNewLightning = EffectBeam(VFX_BEAM_DISINTEGRATE, oHolder, BODY_NODE_CHEST);
