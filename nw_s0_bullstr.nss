...
*/
    ...
    /*
    //Size check
    int nSize = SQLocalsPlayer_GetInt(oTarget, "BULL_STR_SIZE");
    if(nSize == 1)
    {
        float fCurrent = GetObjectVisualTransform(oTarget, OBJECT_VISUAL_TRANSFORM_SCALE);
        float fNew = fCurrent + 0.10;
        SetObjectVisualTransform(oTarget, OBJECT_VISUAL_TRANSFORM_SCALE, fNew, OBJECT_VISUAL_TRANSFORM_LERP_EASE_IN, 1.0);
        DelayCommand(fDuration, SetObjectVisualTransform(oTarget, OBJECT_VISUAL_TRANSFORM_SCALE, fCurrent, OBJECT_VISUAL_TRANSFORM_LERP_EASE_IN, 1.0));
    }
    */
}
