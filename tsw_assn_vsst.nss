...
// End of Spell Cast Hook
    ...
    effect eVis = EffectVisualEffect(VFX_COM_BLOOD_LRG_GREEN);
    ...
    location lVFX;
    int nVFXCounter;
    effect eExplode = EffectVisualEffect(VFX_FNF_GAS_EXPLOSION_ACID, FALSE, 1.5, [0.0,0.0,0.6]);
    while(nVFXCounter < 5)
    {
        fDelay2 = fDelay2 + 0.1;
    ...
    while(GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, OBJECT_SELF) && oTarget != OBJECT_SELF && !GetHasSpellEffect(nSpell,oTarget))
        {
            ...
            if(nDamage > 0)
            {
                  ...
                  effect eFire = EffectDamage(nDOTDamage, DAMAGE_TYPE_ACID);
    ...
    //Sound Effects
    PlaySoundByStrRef(16778119, FALSE);
