...
void ApplyHardcorePenalty(object oDead)
{
    ...
    GiveGoldToCreature(oDead, 2500);

    //Remove faction rep
    int nRep = GetFactionRep(oDead);
    nRep = nRep / 2;
    TakeReputation(oDead, nRep);
    SendMessageToPC(oDead, "You have lost " + IntToString(nRep) + " reputation.");
 ...
 */
    ...
    //Changes sDestTag to new character agartha waypoint, if they died during the prologue
    int nQuest = SQLocalsPlayer_GetInt(oRespawner, "Prologue_Quest");
    if(nQuest < 7)
    {
        sDestTag =  "tp_agartha_start";
    }
    ...
    if(sArea != "OS_training_place")
    {
        ...
        if ( GetLocalInt(oRespawner,"iMarkedForDeath") == 1)
        {
         AssignCommand(oRespawner, JumpToLocation(GetLocation(GetWaypointByTag("dreamspawn"))));
        }
        else {
