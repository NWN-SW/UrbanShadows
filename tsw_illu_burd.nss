...
// End of Spell Cast Hook
    ...
    float fDuration = GetExtendSpell(16.0);
    ...
    eVis = EffectVisualEffect(VFX_FNF_GAS_EXPLOSION_MIND,FALSE,0.75);
    eBoom = EffectVisualEffect(VFX_IMP_PDK_GENERIC_PULSE, FALSE, 0.70, [0.0,0.0,2.0]);
    eBoom2 = EffectVisualEffect(278, FALSE, 0.75, [0.0,0.0,2.0]);
    sElement = "Magi";

    //Apply the explosion at the location captured above.
    ApplyEffectAtLocation(DURATION_TYPE_INSTANT, eBoom, lTarget);
    DelayCommand(0.2, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, eBoom2, lTarget));
    ...
    while (GetIsObjectValid(oTarget))
    {
        if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
        {
            ...
            if(!GetHasSpellEffect(1019, oTarget))
            {
