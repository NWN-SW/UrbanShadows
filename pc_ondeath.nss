...
// * MAY 2002: Just popup the YOU ARE DEAD panel at this point
    ...
    if(sTag == "Fight_Pit")
    {
        ...
        DelayCommand(2.75, ApplyEffectToObject(DURATION_TYPE_INSTANT,EffectHeal(GetMaxHitPoints(oPlayer)), oPlayer));
        DelayCommand(3.0, RemoveEffects(oPlayer));
    }
    else
    {
        DelayCommand(2.5, PopUpDeathGUIPanel(oPlayer, TRUE, TRUE, 0, "Choose an option below. Respawning will incur a loss of 10% of your current gold."));
        SQLocalsPlayer_SetInt(oPlayer, "CURRENTLY_DEAD", 1);
    }

    //Hardcore timer
    int nAmHardcore = SQLocalsPlayer_GetInt(oPlayer, "AM_HARDCORE");
    if(nAmHardcore == 1)
    {
        SendMessageToPC(oPlayer, "You have died as a hardcore character and will respawn automatically in two minutes.");
    }
