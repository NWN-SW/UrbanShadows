            ...
            if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, GetAreaOfEffectCreator()))
            {
                ...
                if(!GetHasSpellEffect(SPELL_ENTANGLE, oTarget))
                {
                    if(!MyResistSpell(GetAreaOfEffectCreator(), oTarget))
                    {
                        ...
                        int nBonusDC =  nDC + GetFocusDC(GetAreaOfEffectCreator(), sElement);
                        //Make reflex save
                        int iDC = GetSpellSaveDC() + 4;
                        int n = MySavingThrow(SAVING_THROW_REFLEX, oTarget, nBonusDC, SAVING_THROW_TYPE_NONE, GetAreaOfEffectCreator() );
                        if(n == 0)
                        {
