...
{
    //Since there are more equipment slots than weapon slots, we want loot weight to be more geared to equipment. 70/30.
    int nRoll = Random(10);
    int nValue;
    if(nRoll >= 0 && nRoll <= 6)
    {
        //Equipment. Weapons are farther down.
        nValue = Random(13);
        switch(nValue)
        {
            case 0:
...
void LootGenerateItems(int nValue, object oPC)
{
    //Create random item in temporary chest.
    ...
    while(nItemLoop < nValue)
    {
        sSetItem = GetRandomItem();
