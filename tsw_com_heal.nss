...
// End of Spell Cast Hook
    ...
    effect eVis2 = EffectVisualEffect(VFX_IMP_PDK_HEROIC_SHIELD);
    effect eAura = EffectVisualEffect(274, FALSE, 2.5);
    ...
    effect eAOE = EffectAreaOfEffect(VFX_MOB_COM_HEAL);
    ...
    eAOE = TagEffect(eAOE, "Commander_Heal");

    //Remove all existing auras
    effect eEffect = GetFirstEffect(OBJECT_SELF);
    while(GetIsEffectValid(eEffect))
    {
        if(GetEffectTag(eEffect) == "Commander_Debuff" || GetEffectTag(eEffect) == "Commander_Slow" || GetEffectTag(eEffect) == "Commander_Resist")
        {
            RemoveEffect(OBJECT_SELF, eEffect);
        }

        if(GetEffectTag(eEffect) == "Commander_Heal")
        {
            RemoveEffect(OBJECT_SELF, eEffect);
            nCount = 1;
