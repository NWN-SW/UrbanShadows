...
const int VFX_IMP_DN_MIRV_EKTO = 837;
...
*/
    ...
    object oTarget = GetFirstObjectInShape(SHAPE_SPHERE, fExplosionRadius, lTarget, TRUE);
    ...
    while (GetIsObjectValid(oTarget))
    {
       ...
       oTarget = GetNextObjectInShape(SHAPE_SPHERE, fExplosionRadius, lTarget, TRUE);
       }
    }
}
   ...
   // SpawnScriptDebugger();
    ...
    if (spellsIsTarget(oTarget, SPELL_TARGET_SELECTIVEHOSTILE, GetAreaOfEffectCreator()))
    {
        ...
        if(!MyResistSpell(GetAreaOfEffectCreator(), oTarget))
        {
            ...
            int nBonusDC = GetFocusReduction(OBJECT_SELF, sElement);
            nDC = nDC + nBonusDC;
...
void DoCamoflage(object oTarget)
{
    ...
    effect eMove = EffectSkillIncrease(SKILL_HIDE, 10);
...
void DoSpikeGrowthEffect(object oTarget)
{
    ...
    if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, GetAreaOfEffectCreator()))
    {
        ...
        {
            int nMetaMagic = GetMetaMagicFeat();
            int nDuration = GetCasterLevel(GetAreaOfEffectCreator())/ 4;
            string sElement = "Acid";
            //Roll damage for each target
            int nDamage = GetThirdLevelDamage(oTarget, nDuration, "AOE");

            //Adjust the damage based on the Reflex Save, Evasion and Improved Evasion.
            int nDC = GetSpellSaveDC();
            int nBonusDC = GetFocusReduction(GetAreaOfEffectCreator(), sElement);
            nDC = nDC + nBonusDC;
           ...
           if (GetHasSpellEffect(SPELL_SPIKE_GROWTH, oTarget) == FALSE)
           {
                ...
                if(!MySavingThrow(SAVING_THROW_REFLEX, oTarget, nDC, SAVING_THROW_ALL, GetAreaOfEffectCreator(), fDelay))
                {
                    ...
                    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eSpeed, oTarget, RoundsToSeconds(nDuration));
                }
           }
        }
    }
...
void DoMissileStorm(int nD6Dice, int nCap, int nSpell, int nMIRV = VFX_IMP_MIRV, int nVIS = VFX_IMP_MAGBLUE, int nDAMAGETYPE = DAMAGE_TYPE_MAGICAL, int nONEHIT = FALSE, int nReflexSave = FALSE)
{
    ...
    while (GetIsObjectValid(oTarget) && nCnt <= nEnemies)
    {
           ...
           (GetObjectSeen(oTarget,OBJECT_SELF)))
        {
                ...
                if(nD6Dice == 1)
                {
                    nDamage = GetFirstLevelDamage(oTarget, nCap, "AOE");
                }
                else if(nD6Dice == 2)
                {
                    nDamage = GetSecondLevelDamage(oTarget, nCap, "AOE");
                }
                else if(nD6Dice == 3)
                {
                    nDamage = GetThirdLevelDamage(oTarget, nCap, "AOE");
                }
                else if(nD6Dice == 4)
                {
                    nDamage = GetFourthLevelDamage(oTarget, nCap, "AOE");
                }
                else if(nD6Dice == 5)
                {
                    nDamage = GetFifthLevelDamage(oTarget, nCap, "AOE");
                }
                else if(nD6Dice == 6)
                {
                    nDamage = GetSixthLevelDamage(oTarget, nCap, "AOE");
                }
                else if(nD6Dice == 7)
                {
                    nDamage = GetSeventhLevelDamage(oTarget, nCap, "AOE");
                }
                else if(nD6Dice == 8)
                {
                    nDamage = GetEighthLevelDamage(oTarget, nCap, "AOE");
                }
                else
                {
                    nDamage = GetNinthLevelDamage(oTarget, nCap, "AOE");
                }
                ...
                nFinalDamage = nFinalDamage + 5;

                //Class mechanics
                DoClassMechanic("Force", "Single", nFinalDamage, oTarget);
...
void doAura(int nAlign, int nVis1, int nVis2, int nDamageType)
{
    ...
    //Roll damage for each target
    int nDamage = GetEighthLevelDamage(oTarget, nCasterLvl, "AoE");
