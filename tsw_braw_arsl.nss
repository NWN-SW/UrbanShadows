...
// End of Spell Cast Hook
    ...
    effect eExplode = EffectVisualEffect(VFX_COM_BLOOD_CRT_RED, FALSE, 2.0, [0.0,0.0,1.5]);
    effect eImpact = EffectVisualEffect(VFX_IMP_PULSE_NEGATIVE, FALSE, 2.0, [0.0,0.0,1.5]);
    effect eVis = EffectVisualEffect(961);
    effect eDam;
    string sTargets;
    string sElement;
    ...
    while (GetIsObjectValid(oTarget))
    {
        if(spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF))
        {
            ...
            if(nDamage > 0)
            {
                ...
                AssignCommand(oTarget, DelayCommand(fDelay, PlaySoundByStrRef(16778131, FALSE)));
            }
        }
    ...
    DoMartialMechanic(sSpellType, sTargets, nFinalDamage, oMainTarget);

    AttackNearest(1.0, oCaster);
}
